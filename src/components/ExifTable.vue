...
onMounted(() => {
  ...
  for (const index in tooltips) {
    ...
    new Tooltip(tooltip_span, outer_span, {
      triggerType: isTouch ? "click" : "hover",
      onShow: () => {
        console.log("A");
      },
    });
    ...
    <table class="table-auto w-full max-w-5xl mx-auto bg-white shadow-lg rounded-lg">
      ...
      <tbody ref="mtable" class="divide-y divide-gray-200">
        ...
        <tr v-for="field in result['exif']" :key="field.key" :data-key="field.key" :data-type="field.typeName"
          ...
          <template v-else>
            <td class="py-3 text-start">{{ field.value }}</td>
          </template>

          <td class="py-3 px-4">
            <div v-if="!field.readonly" class="flex justify-end">
              <template v-if="pageStore.editing_object && field.key === pageStore.editing_object['key']">
